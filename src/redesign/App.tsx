...
function pathPage(path:string):Page{return path.includes('workspace')?'workspace':path.includes('discover')?'discover':path.includes('tasks')?'tasks':path.includes('inbox')?'inbox':path.includes('profile')?'profile':'home'}
export default function App(){
 ...
 <div className="app-content">{page==='home'?<Home {...props}/>:page==='discover'?<Discover {...props}/>:page==='tasks'?<Tasks notify={setToast} profile={profile} pendingDraft={pendingTaskDraft} onDraftConsumed={consumeTaskDraft}/>:page==='inbox'?<Inbox {...props}/>:page==='profile'?<Profile {...props}/>:<Workspace {...props}/>}</div>
