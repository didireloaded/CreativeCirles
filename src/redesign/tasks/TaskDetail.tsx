import { ArrowUpRight, FileText, MessageCircle, Paperclip, X } from 'lucide-react';
...
export default function TaskDetail({ task, onClose, onChange, navigate }: TaskDetailProps) {
  ...
  return <div className="task-detail-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="task-detail" role="dialog" aria-modal="true" aria-labelledby="task-detail-title">
      ...
      {task.source === 'project' && navigate && (
        <button
          type="button"
          className="button secondary"
          style={{ margin: "8px 0 16px", display: "inline-flex", alignItems: "center", gap: 6 }}
          onClick={() => {
            onClose();
            navigate(`projects?item=${task.relatedId || 'between-sand-sky'}`);
          }}
        >
          <ArrowUpRight size={16} /> Open linked project
        </button>
      )}
      {task.source === 'message' && navigate && (
        <button
          type="button"
          className="button secondary"
          style={{ margin: "8px 0 16px", display: "inline-flex", alignItems: "center", gap: 6 }}
          onClick={() => {
            onClose();
            navigate(`inbox?chat=${task.relatedId || 'amara'}`);
          }}
        >
          <ArrowUpRight size={16} /> Open linked conversation
        </button>
      )}
