  ...
  return <div className="ds-page">
    ...
    <dialog className="ds-detail" ref={dialog} onCancel={() => setSelected(null)} onClose={() => setSelected(null)} onClick={event => { if (event.target === event.currentTarget) closeDetail(); }} aria-labelledby="ds-detail-title">
      {selected && <div className="ds-detail-inner">
        ...
        <div className="ds-detail-body"><button className="icon-button ds-detail-close" onClick={closeDetail} aria-label="Close work detail" autoFocus><X size={22} /></button><p className="eyebrow">{selected.category}</p><h2 id="ds-detail-title">{selected.title}</h2><p className="ds-detail-description">{selected.description}</p><div className="ds-detail-author"><img className="avatar" src={creators[selected.creator].image} alt="" /><div><strong>{creators[selected.creator].name}</strong><span>{creators[selected.creator].role}</span></div></div><button className={`button ${saved.includes(selected.id) ? 'secondary' : 'primary'} ds-detail-save`} aria-pressed={saved.includes(selected.id)} onClick={() => saveWork(selected)}><Bookmark size={18} fill={saved.includes(selected.id) ? 'currentColor' : 'none'} />{saved.includes(selected.id) ? 'Saved to inspiration' : 'Save to inspiration'}</button><p className="ds-detail-note">Saved items stay on this device.</p></div>
      </div>}
    </dialog>
